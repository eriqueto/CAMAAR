...
**Escopo da Sprint 2:** O objetivo principal desta sprint é a **implementação em código** das features cujos cenários BDD foram especificados na Sprint 1. Isso inclui o desenvolvimento da lógica no sistema, a criação e execução dos respectivos testes em RSpec com sucesso, a documentação atualizada nesta Wiki e a gestão visual do progresso utilizando o Kanban no GitHub.
...
## 📋 Funcionalidades, Responsáveis e Estimativas (Implementação)
...
**🚀 Velocity Total Planejado da Sprint 2:** 56 pontos

---

## 📜 Regras de Negócio por Funcionalidade (Validadas no RSpec)
*(As regras abaixo nortearam o desenvolvimento e os testes desta sprint)*
...
## 🔀 Política de Branching, Kanban e Pull Request (Entregas da Sprint 2)

A equipe utilizará a estratégia de ramificação exigida para a entrega da avaliação.

* **Quadro Kanban:** O gerenciamento das tarefas foi feito distribuindo as issues entre as lanes: *Backlog* - *Ready* - *In Progress* - *In Review* - *Done*, utilizando a interface de projetos do GitHub.
* **Fluxo de Integração e Release:** Para esta etapa, a equipe não utilizou branches auxiliares (individuais). Todo o desenvolvimento e os commits de todos os membros foram realizados diretamente na branch de release da sprint, cujo nome é **`sprint-2`**. Nenhuma modificação ou commit deve ser feito nesta branch após a abertura do Pull Request.

### Instruções de Abertura do Pull Request
A submissão oficial do código segue as seguintes diretrizes para a criação do PR no GitHub:
1. Acessar a branch e clicar no botão "New pull request" na tela "Comparing changes".
2. **Base repository:** Definir como `EngSwCIC/CAMAAR` apontando para a base `main`.
3. **Head repository:** Definir como `<fork_do_grupo>/CAMAAR` e realizar o compare com a branch `sprint-2`.
4. **Title:** Preencher com a identificação geral: `Grupo 11 - Entrega da Sprint 2`.
5. **Leave a comment:** Informar no corpo do PR o nome completo e a matrícula de todos os responsáveis pela branch, bem como a lista de issues e features que foram implementadas.